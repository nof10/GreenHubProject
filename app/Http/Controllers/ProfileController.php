        ...
        if ($user->typeuser === 'client') {
            ...
            $validated = $request->validate([
                ...
                'email'       => [
                    'required',
                    'email',
                    Rule::unique('drive__profiles', 'email')->ignore($user->id, 'driver_id'),
                ],
                'city'        => 'nullable|string|max:100',
                'national_ID' => [
                    'nullable',
                    'string',
                    'size:10',
                    Rule::unique('drive__profiles', 'national_ID')->ignore($user->id, 'driver_id'),
                ],
                'phone'       => [
                    'nullable',
                    'string',
                    'max:20',
                    Rule::unique('drive__profiles', 'phone')->ignore($user->id, 'driver_id'),
                ],
